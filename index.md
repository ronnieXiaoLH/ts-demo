...
**装饰器工厂：** 装饰器工厂是生成装饰器的函数，它返回实际用于装饰的装饰器。装饰器工厂主要作用是可以给装饰器传参。
...
**元数据：** `reflect-metadata` 是一个用于处理元数据（metadata）的库，它允许在运行时为类和类成员（如属性、方法等）添加元数据，并在需要时检索这些元数据。

**元数据内置的三个常用元数据键：**

- `design:type` 用于存储属性的类型信息。可以通过 `Reflect.getMetadata('design:type', target, propertyKey)` 来检索属性的类型。
- `design:paramtypes` 用于存储方法参数的类型信息。可以通过 `Reflect.getMetadata('design.paramtypes', target, propertyKey)` 来检索方法参数的类型数组。
- `design:returntype` 用于存储方法返回值的类型信息。可以通过 `Reflect.getMetadata('design.returntype', target, propertyKey)` 来检索方法返回值的类型。
...
interface 和 type 在声明文件中不需要在前面添加 declare 关键字，因为在 js 中，是没有 interface 和 type 关键字的，所以它能够被识别。
